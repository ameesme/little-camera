...
- **Sleep:** light sleep, entered once the radio window behind the sleep face closes, woken by the shutter (HIGH level) or a `BREATH_MS` timer (the face's breath; the VCOM keep-alive rides it, and a static_assert keeps it inside `Display::VCOM_INTERVAL_MS`). The sleep loop leaves **only on a GPIO wake held for `WAKE_HOLD_MS`** (a tap chirps and goes straight back to sleep); a timer wake toggles VCOM, breathes the face and services the mood (a due chirp plays right there, radio and camera being down), and any other cause (BT, undefined, a rejected `esp_light_sleep_start()`) is logged and slept through — treating "anything but the timer" as a press made the camera wake by itself once the radio existed. `Sync::end()` forces the BT controller off and clears its wake source before sleeping for the same reason. Two more things are load-bearing — don't remove them:
...
- **Mood is untested on hardware** — the scheduler is host-tested over a simulated month, the faces are previewed, but the first real week (NVS resume, chirps out of light sleep, the breath's power cost — estimated at roughly +0.2mA average for the face rows every second, on a ~0.3mA idle, unmeasured) is still ahead. `-DLC_MOOD_FAST` compresses it to a bench session.
