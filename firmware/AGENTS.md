...
- **UI mode machine** (`Mode` in main.cpp). One button drives everything, so what a press means depends entirely on the screen. Gesture table: **press = shoot**, **hold 700ms (`GALLERY_HOLD_MS`) = gallery in/out**, **hold 1500ms (`TRASH_HOLD_MS`) = trash** on the save screen, **hold 1000ms (`WAKE_HOLD_MS`) = wake** from the doze or from sleep (taps are ignored there). Every hold compares against a fresh `millis()` (see the Save note) and fires *on the threshold*, not on release.
  ...
  - `Dozing` — what the idle timeout leads to. The screen being left up is redrawn once without the hint (gallery stays gallery, anything else becomes a last live frame) and the panel then holds it; the camera driver is torn down; the radio keeps advertising. Nothing is drawn during the doze except that a pairing code wakes the camera. Hold `WAKE_HOLD_MS` returns to the retained screen. After `DOZE_MS` (30s), stretched while a phone is active (`Sync::activeRecently`) or the console is in use, `enterSleepMode()` runs as before.
...
- **BLE sync** (`sync.{h,cpp}`, layout in `sync_protocol.h`, contract in `docs/protocol.md` §3): NimBLE-Arduino 2.x peripheral, one service, four characteristics (Info / Secret / Control / Data). The phone lists, pulls files as notification frames, and ACKs; an ACK renames the file to its `s` form. Rules:
  ...
  - **When the radio runs:** while awake and through the 30s doze after the idle timeout (`DOZE_MS` in main.cpp); the doze stretches while a connected phone has done something in the last 60s (`Sync::activeRecently`, `BLE_ACTIVE_MS` in sync.cpp). The shutter always wins; a transfer just continues alongside.
