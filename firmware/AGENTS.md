...
- **UI mode machine** (`Mode` in main.cpp). One button drives everything, so what a press means depends entirely on the screen. Gesture table: **press = shoot**, **hold 700ms (`GALLERY_HOLD_MS`) = gallery in/out**, **hold 1500ms (`TRASH_HOLD_MS`) = trash** on the save screen, **hold 1000ms (`WAKE_HOLD_MS`) = wake** a sleeping camera (taps are ignored; the wake fires at the one-second mark, not on release). Every hold compares against a fresh `millis()` (see the Save note) and fires *on the threshold*, not on release.
  ...
  - `Updating` — new firmware coming in over BLE (see the update bullet below): a progress bar owns the screen, the button does nothing, the idle timeout is suspended and the camera driver is torn down for the duration. It ends in a reboot into the new image, or in an outcome line for `UPDATE_RESULT_MS` before the screen goes back where it came from — the sleep face if that is what was up, otherwise the viewfinder.
...
- **Version:** `version.h` holds `LC_VERSION_*`, built into `LC_VERSION_STRING` from the numbers so the two cannot drift. It goes out in Info (protocol §3.1) and `stat`, and the server compares a release against it — so bump it in the same commit as the change it names.
- **Host checks:** `tools/hosttest/make test` runs the Arduino-free headers (PBM header codec, CRC-32, SHA-256 one-shot and streaming, short code, the sync and update packers, chirp composer, a month of the mood machine) against known vectors; `make check` syntax-compiles audio/mood_store/storage/console/identity/sync/main against stubbed Arduino headers. Neither replaces `pio run`, but both run anywhere.
- **BLE sync** (`sync.{h,cpp}`, layout in `sync_protocol.h`, contract in `docs/protocol.md` §3): NimBLE-Arduino 2.x peripheral, one service, four characteristics (Info / Secret / Control / Data). The phone lists, pulls files as notification frames, and ACKs; an ACK renames the file to its `s` form. Rules:
  ...
  - **Update** (characteristic `06`) is `WRITE_NR | WRITE_ENC | WRITE_AUTHEN`: without response because an ATT acknowledgement per 500 bytes would halve the throughput of a megabyte, and encrypted + authenticated because replacing the firmware is the most powerful thing this service can be asked to do. `UPDATE_BEGIN` re-checks that the link is bonded before opening a session.
...
- **Firmware update** (`ota.{h,cpp}`, the BLE half in `sync.cpp`, contract in `docs/protocol.md` §3.7): the phone pushes an image to the app slot the camera is not running from. What is load-bearing here:
  - **`default_8MB.csv` already has two 3.3MB app slots** and only one was ever used. That is the whole reason this fits without touching the partition table — which stays frozen, photos being in `spiffs` either way.
  - **Nothing is armed until everything checks out.** `esp_ota_set_boot_partition()` runs only after every announced byte arrived, the SHA-256 of the stream matched the digest in `UPDATE_BEGIN`, and `esp_ota_end()` accepted the image. A partial flash therefore cannot be booted; the running slot is untouched and still bootable.
  - **The first boot is a trial.** `Ota::begin()` (early in `setup()`, before anything slow) counts it in NVS; `Ota::confirm()` ends it once the loop has run for `OTA_CONFIRM_MS` (20s). An image that cannot manage that is rolled back — by the bootloader if the core was built with `CONFIG_BOOTLOADER_APP_ROLLBACK_ENABLE`, by the NVS counter if it wasn't. Don't make the health check depend on anything a rollback cannot fix (the filesystem, the sensor): the only thing it can fix is an image that will not stay up.
  - `esp_ota_begin()` is called with **`OTA_WITH_SEQUENTIAL_WRITES`**, so each write erases at most one 4KB sector. Erasing 3.3MB up front blocks the main loop for seconds and the radio dies under it.
  - **Chunks are staged, never written from the BLE task.** `sync.cpp` keeps a six-slot ring (`UPDATE_SLOTS`), the callback copies and publishes with a release store, and `Sync::loop()` writes flash. The window the camera grants is exactly the ring, so a phone that keeps to it cannot be dropped; one that is dropped anyway is told which offset to rewind to, because every chunk carries its own.
  - **No sleep while it runs.** `Mode::Updating` is excluded from the idle timeout. Light sleep mid-image would take the radio down under the transfer and strand a half-written slot.
  - **The way back after an update is `ota revert`** on the USB console. `pio run -t upload` writes the *first* app slot, but the bootloader is pointed at the second, so without reverting (or `esptool.py erase_region 0xe000 0x2000`) the board keeps booting what the phone sent. Photos survive either way.
  - Verified so far: `tools/hosttest` (the packers, the streaming SHA-256) and `make check`. **Not yet verified on hardware**: anything at all — the erase-as-you-go write rate against the window, iOS write-without-response throughput, the rollback path with this core's bootloader, and whether `esp_ota_get_state_partition()` reports `PENDING_VERIFY` at all in an Arduino build. Test the rollback first, with an image that panics on purpose.
...
scenes:   viewfinder  capture  save  slide  toast  sleep  splash  gallery  gallery-empty
          pairing  pairing-done  update  update-done  update-failed
  ...
  --percent N  progress 0-100 ('update' scene, default 42)
```
...
- **Firmware update is untested on hardware**, and it is the one feature whose failure mode is a camera that will not boot. Before trusting it: check that the second app slot is where the partition table says, push an image that deliberately panics and watch it roll back, then push a good one. `Ota::confirm()` is what stands between a bad image and a brick, so exercise the path where it is never reached.
