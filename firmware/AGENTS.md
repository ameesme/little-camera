...
- **Audio** (`audio.{h,cpp}`): three kinds of sound, two ways of making them. The **click** (shutter, wake, back to the viewfinder; a lighter tick into the gallery) and the **chirps** are bit-banged square waves on the buzzer pin (`digitalWrite` + `delayMicroseconds`, blocking) because the LEDC `tone()` path produced nothing for short sounds right after a light-sleep wake. The **melodies** (ta-da-da, its reverse for sleep, the chirp-chirp, the A-A'-E "saved") still use `tone()` from the async `Audio::update()` machine. Nothing is longer than 300ms; ta-da-da is exactly that and it is the ceiling for everything else.
  - **Chirps** are composed, not listed: `chirp.h` (Arduino-free) turns a happiness level 0-255 plus a seed into a 1-3 segment score of tones, sweeps and sample-and-hold noise, on the A major scale and rising when happy, detuned, off-scale, falling and noisy when sad; `Audio::playChirp()` plays it. The behaviour this serves (the camera wanting attention over a week without a photo) is in `docs/mood.md`; the scheduler and the sleep-face changes there are not built yet. **Until they are, a tap in the gallery plays a chirp at a random happiness** and logs it (`Gallery: chirp happiness=173/255 band=2 T1661/44 +12 S1661>2637/80 = 136ms`); the light tick returns when the scheduler lands.
...
- **Host checks:** `tools/hosttest/make test` runs the Arduino-free headers (PBM header codec, CRC-32, SHA-256, short code, chirp composer) against known vectors; `make check` syntax-compiles audio/storage/console/identity/sync/main against stubbed Arduino headers. Neither replaces `pio run`, but both run anywhere.
...
- **Mood scheduler and sleep-face moods** (`docs/mood.md`) are specified, not built. The chirp engine is; the gallery tap is its temporary test hook.
