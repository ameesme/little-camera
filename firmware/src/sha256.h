...
// Compact SHA-256, header-only, no dependencies. The firmware hashes exactly
// one thing with it (the 12-character camera id, to derive the short code), so
// pulling in mbedtls on the target and something else on the host would be
// more code than this is. Public-domain algorithm per FIPS 180-4.
...
inline void block(uint32_t state[8], const uint8_t chunk[64]) {
    uint32_t w[64];
    for (int i = 0; i < 16; i++) {
